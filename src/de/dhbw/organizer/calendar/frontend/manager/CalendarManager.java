...
public class CalendarManager {
	 ...
	 * @return
	 */
	public EventAdapter getCalendarDays(Context context, String CalendarName) {
		mIndexAlreadySet = false;
		...
		List<CalendarEvent> listOfEvents = new ArrayList<CalendarEvent>();
		//List<CalendarDay> listOfDays = new ArrayList<CalendarDay>();
		...
		while (cur.moveToNext()) {
			...
			if ((eventdtend >= System.currentTimeMillis()) && (!mIndexAlreadySet)) {
				...
				CalendarEvent c1 = new CalendarEvent(eventName, eventdtstart, eventdtend, eventLocation, eventDescription, true);
				CalendarEvent c2 = new CalendarEvent(eventName, eventdtstart, eventdtend, eventLocation, eventDescription, true);
				ArrayList a1 = new ArrayList<CalendarEvent>();
				a1.add(c1);
				a1.add(c2);
				
				//CalendarDay d1 = new CalendarDay(a1,"Dienstag");
				
				//listOfDays.add(d1);
				
				
				//listOfDays.add( new ArrayList<CalendarEvent>(new CalendarEvent(eventName, eventdtstart, eventdtend, eventLocation, eventDescription, true)));
			} else {
				//listOfDays.add(new CalendarEvent(eventName, eventdtstart, eventdtend, eventLocation, eventDescription, false));
		...
		//DayAdapter adapter = new DayAdapter(context, listOfDays);
		cur.close();
		return adapter;
	}
